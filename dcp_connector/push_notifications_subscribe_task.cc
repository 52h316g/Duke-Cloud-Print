...
int PushNotificationsSubscribeTask::ProcessStart() {
    std::cout << "Push notifications: Subscription task started."<< std::endl;
  talk_base::scoped_ptr<buzz::XmlElement> iq_stanza(
      MakeSubscriptionMessage(subscriptions_, GetClient()->jid(),
                              task_id()));
  std::cout << "Push notifications: Subscription stanza: "
          << XmlElementToString(*iq_stanza.get())<< std::endl;
...
int PushNotificationsSubscribeTask::ProcessResponse() {
  std::cout << "Push notifications: Subscription response received." << std::endl;
  const buzz::XmlElement* stanza = NextStanza();
  if (stanza == NULL) {
    return STATE_BLOCKED;
  }
  std::cout << "Push notifications: Subscription response: "
